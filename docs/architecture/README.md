# Architecture Docs Index

## Core

- [System Design](./system-design.md)
- [Repository Layout](./repository-layout.md)
- [API Contract](./api-contract.md)
- [Database Schema](./database-schema.md)
- [pgvector Retrieval Design](./pgvector-retrieval-design.md)
- [Agent Policy](./agent-policy.md)
- [LLM Provider Integration](./llm-provider.md)
- [Reranker Provider Integration](./reranker-provider.md)
- [Eval Methodology](./eval-methodology.md)
- [Observability](./observability.md)

## Iteration History

- [Step 2 Backend RAG Loop](./step2-backend-rag-loop.md)
- [Step 4 Agent Policy](./step4-agent-policy.md)
- [Step 5 Observability Hardening](./step5-observability-hardening.md)
- [Step 6 Eval Regression CI](./step6-eval-regression-ci.md)
