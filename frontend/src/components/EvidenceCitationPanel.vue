...
<template>
  <Card>
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h3 class="text-sm font-semibold text-slate-900">Evidence / Citation</h3>
    ...
    <div v-else class="mt-3 grid gap-4 lg:grid-cols-[300px_1fr]">
      <ul class="space-y-2" data-testid="citation-list">
        <li
          ...
          :class="selectedChunkId === citation.chunk_id ? 'border-primary bg-sky-50' : 'border-slate-200 bg-slate-50'"
        >
          <button class="w-full text-left" type="button" @click="selectCitation(citation.chunk_id)">
            ...
            <p class="mt-2 line-clamp-2 text-sm text-slate-800">{{ citation.quote }}</p>
      ...
      >
        <div class="flex flex-wrap items-center justify-between gap-2">
          <h4 class="text-sm font-semibold text-slate-900">Selected Evidence</h4>
        ...
        <div class="mt-3 rounded-md border border-slate-200 bg-slate-50 p-3">
          <p class="text-xs uppercase tracking-wide text-slate-500">Citation Quote</p>
          <p class="mt-1 whitespace-pre-wrap text-sm text-slate-800">{{ selectedCitation.quote }}</p>
        </div>

        <div v-if="selectedRetrieval" class="mt-3 rounded-md border border-slate-200 bg-slate-50 p-3">
          <p class="text-xs uppercase tracking-wide text-slate-500">Retrieved Chunk Preview</p>
          <p class="mt-1 whitespace-pre-wrap text-sm text-slate-700">{{ selectedRetrieval.content_preview }}</p>
        </div>
      </section>
    </div>
  </Card>
